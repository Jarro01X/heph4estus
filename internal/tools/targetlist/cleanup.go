...
const (
	tempDirPrefix       = "heph-targetlist-"
	DefaultStaleTempAge = 24 * time.Hour
)

// CleanupStaleTempDirs removes stale target-list temp directories from os.TempDir.
...
func cleanupStaleTempDirs(root string, now time.Time, maxAge time.Duration) error {
	...
	for _, entry := range entries {
		...
		info, err := entry.Info()
		if err != nil {
			errs = append(errs, fmt.Errorf("stat stale target-list temp dir %s: %w", filepath.Join(root, entry.Name()), err))
			continue
		}
		...
		if err := os.RemoveAll(path); err != nil {
			errs = append(errs, fmt.Errorf("removing stale target-list temp dir %s: %w", path, err))
		}
	}
