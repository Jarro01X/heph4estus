...
func TestCleanupStaleTempDirsRemovesOnlyStaleTargetListDirs(t *testing.T) {
	root := t.TempDir()
	now := time.Now()
	stale := filepath.Join(root, "heph-targetlist-stale")
	fresh := filepath.Join(root, "heph-targetlist-fresh")
	...
	if _, err := os.Stat(stale); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("stale target-list dir should be removed, stat err=%v", err)
	}
	if _, err := os.Stat(fresh); err != nil {
		t.Fatalf("fresh target-list dir should remain: %v", err)
	}
...
func TestCleanupStaleTempDirsIgnoresFilesAndSymlinks(t *testing.T) {
	...
	filePath := filepath.Join(root, "heph-targetlist-file")
	...
	link := filepath.Join(root, "heph-targetlist-link")
...
func TestCleanupStaleTempDirsDefaultsMaxAge(t *testing.T) {
	...
	if _, err := os.Stat(stale); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("stale target-list dir should be removed with default age, stat err=%v", err)
	}
}
