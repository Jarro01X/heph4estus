...
output "nats_url" {
  ...
  value       = "${module.controller.nats_tls_enabled ? "tls" : "nats"}://${local.nats_operator_user}:${local.nats_operator_password}@${linode_instance.controller.ip_address}:4222"
  sensitive   = true
}
...
output "s3_endpoint" {
  ...
  value       = "${module.controller.minio_tls_enabled ? "https" : "http"}://${linode_instance.controller.ip_address}:9000"
}
